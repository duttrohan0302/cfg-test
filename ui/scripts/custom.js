...
$(function() {
	...
	function init_template(){
        ...
        if($("#page").hasClass("Dashboard")){
          console.log("Here For Dashboard");
          var url = window.location.href;
          var id = url.split("#NANM/");
          if (id.length>=2) {
            id = id[1];
          }
        }
        if($("#page").hasClass("News")){
          console.log("Here For News");
          var url = window.location.href;
          var id = url.split("#NANM/");
          if (id.length>=2) {
            id = id[1];
        ...
        setTimeout(function(){
            $('[data-toggle-theme]').off('click').on('click',function(e){
                ...
                if($('body').hasClass('theme-light')){eraseCookie('PatialaCity_dark_mode'); createCookie('PatialaCity_light_mode', true, 1);}
                if($('body').hasClass('theme-dark')){eraseCookie('PatialaCity_light_mode'); createCookie('PatialaCity_dark_mode', true, 1);}
                //e.preventDefault;
                return false;
            });
        },1000);

        if (readCookie('PatialaCity_dark_mode')) {$('body').removeClass('theme-light').addClass('theme-dark');}
        if (readCookie('PatialaCity_light_mode')) {$('body').removeClass('theme-dark').addClass('theme-light');}
      ...
      $('.single-slider-full').owlCarousel({loop:true, margin:0, nav:false, lazyLoad:true, items:1, autoplay: false, stagePadding:0, autoplayTimeout:4000});
        ...
        setTimeout(function(){
            ...
            //var share_link = 'https://patiala.city';
            var share_title = 'Patiala.City | Connect With Your Community';
		...
		countdown('05/01/2020 01:00:00 PM');
        ...
        function activate_pageGreeting(){
            ...
            var greetingMessage;
            var greetingUser = getLocalStorage("UserInputName");//$('.greeting-text').data('username')
        ...
        function highlight_colors(){
            ...
            if (data.indexOf(bodyColor) > -1) {
                ...
                if(readCookie("PatialaCity_highlight_color")!=null){
                  var highlightColorCode = readCookie("PatialaCity_highlight_color") // ck_work : Reading cookie to use selected highlight
                }else {
                  var highlightColorCode = colorsArray[highlightLocated][2]
                }
                // read cookie for color here .... ck_work
        ...
        $('body').on('click','[data-change-highlight]',function(changeColor){
            ...
            var data = colorsArray.map(colorsArray => colorsArray[0]);
                ...
                createCookie('PatialaCity_highlight_color', highlightColorCode, 1); // ck_work : Creating cookie to store selected highlight
                var highlightColor = '.color-highlight{color:'+highlightColorCode+'!important}'
                var highlightBg = '.bg-highlight{background-color:'+highlightColorCode+'!important}'
